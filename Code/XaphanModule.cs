    ...
    public class XaphanModule : EverestModule
    {
        ...
        private void onLevelLoad(Level level, Player.IntroTypes playerIntro, bool isFromLoader)
        {
            ...
            if (level.Session.Area.GetLevelSet() == "Xaphan/0")
            {
                ...
                {
                    if (ModSaveData.SoCMVer < 300)
                    {
                        // Remove Bombs upgrade

                        Commands.Cmd_Remove_Upgrades("Bombs");
                        ...
                        for (int i = SaveData.Instance.GetLevelSetStats().AreaOffset; i < SaveData.Instance.GetLevelSetStats().AreaOffset + SaveData.Instance.GetLevelSetStats().Areas.Count; i++)
                        {
                            ...
                            foreach (KeyValuePair<string, string> oldRoom in RoomsNamesConversion)
                            {
                                ...
                                foreach (string visitedRoomTile in ModSaveData.VisitedRoomsTiles)
                                {
                                ...
                                foreach (string unlockedWarp in ModSaveData.UnlockedWarps)
                                {
                            ...
                            {
                                ModSaveData.VisitedRooms.Remove(visitedRoom);
                            }
                            foreach (string visitedRoomTile in oldVisitedRoomsTiles)
                            {
                                ModSaveData.VisitedRoomsTiles.Remove(visitedRoomTile);
                            }
                            foreach (string unlockedWarp in oldUnlockedWarps)
                            {
                                ModSaveData.UnlockedWarps.Remove(unlockedWarp);
                            }
                            foreach (string visitedRoom in newVisitedRooms)
                            {
                                ModSaveData.VisitedRooms.Add(visitedRoom);
                            }
                            foreach (string visitedRoomTile in newVisitedRoomsTiles)
                            {
                                ModSaveData.VisitedRoomsTiles.Add(visitedRoomTile);
                            }
                            foreach (string unlockedWarp in newUnlockedWarps)
                            {
                                ModSaveData.UnlockedWarps.Add(unlockedWarp);
                            }
                        }
                    }
                }
                ModSaveData.SoCMVer = SoCMVersion.Major * 100 + SoCMVersion.Minor * 10 + SoCMVersion.Build;
            }
